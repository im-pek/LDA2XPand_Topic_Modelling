...
topics = np.arange(X.shape[0]) #['fruit doc', 'animal doc', 'mixed doc'] #10 any-given topic names
...
for topic_n,topic in enumerate(topics):
    ...
    print ('\n')
    words2,mis = zip(*topic)
    print ('Corex:')
    print(list(words2))
    print ('\n')
    print ('Word2Vec:')
    top_wordsss=model.wv.most_similar(positive=list(words2), topn=30)#, negative=['good']))#, topn=20)) #!
    top_MI_values_list=[]
